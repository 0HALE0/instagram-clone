...
export default async function BookmarkedPage(){
    ...
    const bookmarks = await prisma.bookmark.findMany({where:{email:session?.user?.email as string},});
    const posts = await prisma.post.findMany({where:{id:{in: bookmarks.map(b=>b.postId)},}});
    return( 
        <div>
             <ProfilePageInfo profile={profile} isOurProfile={true} ourFollow={null}/>
             <ProfileNav isOurProfile={true} username={profile.username || ''}/>
             <div className="mt-4"><PostsGrid posts={posts} /></div>
